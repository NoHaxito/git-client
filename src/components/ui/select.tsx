...
function SelectItem({
  ...
  const isLeft = checkPosition === "left";

  return (
    <SelectPrimitive.Item
      className={cn(
        "flex in-data-[side=none]:min-w-[calc(var(--anchor-width)+1.25rem)] cursor-default items-center justify-between gap-2 gap-4 rounded-sm py-1 text-base outline-none data-disabled:pointer-events-none data-highlighted:bg-accent data-highlighted:text-accent-foreground data-disabled:opacity-64 sm:text-sm [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        isLeft ? "ps-2 pe-2" : "ps-2 pe-2",
        className
      )}
      data-slot="select-item"
      {...props}
    >
      <SelectPrimitive.ItemText className="min-w-0">
        {children}
